...
struct Sphere {
	...
	float radius;
	int mat;
	Sphere(const vec3& _center, float _radius, int anyags) { center = _center; radius = _radius; mat = anyags; }
...
public:
	void build() {
		...
		for (int i = 0; i < 20; i++) objects.push_back(new Sphere(vec3(rnd() - 0.5, rnd() - 0.5, rnd() - 0.5), rnd() * 0.1, 0));
		for (int i = 0; i < 40; i++) objects.push_back(new Sphere(vec3(rnd() - 0.5, rnd() - 0.5, rnd() - 0.5), rnd() * 0.1, 1));
